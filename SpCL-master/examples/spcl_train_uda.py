...
from examples.otla_tool import save_image_label
...
def main_worker(args):
    ...
    for epoch in range(args.epochs):
        ...
        train_pseudo_label = []
        train_real_label = []
        for i, ((fname, real_label, cid), label) in enumerate(zip(sorted(dataset_target.train), pseudo_labels)):
            ...
            if ((indep_score<=indep_thres) and (comp_score.item()<=cluster_R_comp[label.item()-source_classes])):
                ...
                train_pseudo_label.append(label)
            else:
                ...
                outliers+=1
                train_pseudo_label.append(source_classes+len(cluster_R_indep)+outliers)

            train_image_path.append(fname)
            train_real_label.append(real_label)

        if "sysumm01" in args.dataset_target:
            save_image_label(train_image_path, train_pseudo_label, train_real_label, model, epoch, logs_dir=args.logs_dir, save_path=osp.join(args.data_dir, "SYSU-MM01"),
                             img_size=(144, 288), source_domain=args.dataset_source, target_domain=args.dataset_target, method_name="spcl_uda")
        elif "regdb" in args.dataset_target:
            save_image_label(train_image_path, train_pseudo_label, train_real_label, model, epoch, logs_dir=args.logs_dir, save_path=osp.join(args.data_dir, "RegDB"),
                             img_size=(144, 288), source_domain=args.dataset_source, target_domain=args.dataset_target, method_name="spcl_uda")
        ##########################################################
